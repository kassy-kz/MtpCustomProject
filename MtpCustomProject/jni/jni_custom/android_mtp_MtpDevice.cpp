...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
    ...
    ALOGE("register android mtp mtpdatabase\n");
    if (register_android_mtp_MtpDatabase(env) < 0) {
        ALOGE("ERROR: MtpDatabase native registration failed");
        goto bail;
    }

    ALOGE("register android mtp mtpdevice\n");
...
android_mtp_MtpDevice_set_shutter_speed(JNIEnv *env, jobject thiz, jint value){
    ...
    ALOGE("jni android_mtp_MtpDevice_set shutter speed value = %d", value);
...
int register_android_mtp_MtpDevice(JNIEnv *env)
{
    ...
    clazz = env->FindClass("android/mtp/custom/MtpDevice");
    if (clazz == NULL) {
        ALOGE("Can't find android/mtp/custom/MtpDevice");
        return -1;
    }
    ...
    return AndroidRuntime::registerNativeMethods(env,
                "android/mtp/custom/MtpDevice", gMethods, NELEM(gMethods));
}
